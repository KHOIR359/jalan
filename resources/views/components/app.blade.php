...
<body class=" h-screen antialiased leading-none font-sans {!!$attributes['class']!!}">
            ...
            <div class="container flex items-center justify-between px-6 mx-auto" x-data="{show:false}">
                <div>
                    <div class="flex items-center">
                        ...
                        <a href="{{ url('/') }}" class="text-xl font-bold text-blue-500 no-underline">
                            {{ config('app.name', 'Laravel') }}
                        </a>
                    </div>
                </div>
                <button class="p-2 border border-gray-300 rounded shadow outline-none cursor-pointer focus:ring-none focus:outline-none ring-none md:hidden" @click="show=!show"><i class="text-gray-800 fa-fw fa fa-bars md:hidden"></i></button>

                <nav @click.away="show=false" x-show="show" class="absolute z-50 px-4 py-3 space-y-2 text-sm text-gray-900 bg-white border border-gray-300 rounded-lg shadow lg:block sm:text-base top-20 right-4 md:hidden"
                ...
                <nav class="hidden space-x-2 text-sm text-gray-800 md:block sm:text-base top-20 right-4"
                     x-transition:enter="transition ease-out duration-300"
                     x-transition:enter-start="opacity-0 transform scale-60 skew-x-6"
                     x-transition:enter-end="opacity-100 transform scale-100 skew-x-0"
                     x-transition:leave="transition ease-in duration-300"
